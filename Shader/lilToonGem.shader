Shader "lilToonGem"
{
    Properties
    {
        ...
        _RefractionFresnelPower ("sRefractionFresnel", Range(0.01, 10)) = 1
        _GemChromaticAberration ("sChromaticAberration", Range(0, 1)) = 0.02
        _GemEnvContrast ("sContrast", Float) = 2
        [HDR] _GemEnvColor ("sEnvironmentColor", Color) = (1,1,1,1)
        _GemParticleLoop ("sParticleLoop", Float) = 8
        [HDR] _GemParticleColor ("sColor", Color) = (4,4,4,1)
        _GemVRParallaxStrength ("sVRParallaxStrength", Range(0, 1)) = 1
        // Rendering
        _Cull ("sCullModes", Int) = 0
        _SrcBlend ("sSrcBlendRGB", Int) = 1
        _DstBlend ("sDstBlendRGB", Int) = 1
